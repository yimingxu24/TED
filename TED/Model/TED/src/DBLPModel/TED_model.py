...
class InstanceAggLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout, alpha, device):
        ...
        self.P_author = nn.Parameter(torch.zeros(size=(in_dim, out_dim)))
        nn.init.xavier_uniform_(self.P_author.data, gain=1.414)
        self.P_phrase = nn.Parameter(torch.zeros(size=(in_dim, out_dim)))
        nn.init.xavier_uniform_(self.P_phrase.data, gain=1.414)
        self.P_venue = nn.Parameter(torch.zeros(size=(in_dim, out_dim)))
        nn.init.xavier_uniform_(self.P_venue.data, gain=1.414)
        self.P_year = nn.Parameter(torch.zeros(size=(in_dim, out_dim)))
        nn.init.xavier_uniform_(self.P_year.data, gain=1.414)

        self.W_AA = nn.Parameter(torch.zeros(size=(out_dim * 2, out_dim)))
        nn.init.xavier_uniform_(self.W_AA.data, gain=1.414)
        self.W_PAA = nn.Parameter(torch.zeros(size=(out_dim * 3, out_dim)))
        nn.init.xavier_uniform_(self.W_PAA.data, gain=1.414)
        self.W_VAA = nn.Parameter(torch.zeros(size=(out_dim * 3, out_dim)))
        nn.init.xavier_uniform_(self.W_VAA.data, gain=1.414)
        self.W_YAA = nn.Parameter(torch.zeros(size=(out_dim * 3, out_dim)))
        nn.init.xavier_uniform_(self.W_YAA.data, gain=1.414)
        self.W_PPAA = nn.Parameter(torch.zeros(size=(out_dim * 4, out_dim)))
        nn.init.xavier_uniform_(self.W_PPAA.data, gain=1.414)

        self.leakyrelu = nn.LeakyReLU(alpha)

    def forward(self, author_feats, phrase_feats, venue_feats, year_feats, trans_adj_list, pattern_name):   
        f_author = torch.mm(author_feats, self.P_author)  # 投影 [300] -> [50]
        if phrase_feats != None or  venue_feats != None or year_feats != None:
            f_phrase = torch.mm(phrase_feats, self.P_phrase)
            f_venue = torch.mm(venue_feats, self.P_venue)
            f_year = torch.mm(year_feats, self.P_year)

        if pattern_name == 'PAA':
            concat = torch.cat([f_author[trans_adj_list[0]], f_author[trans_adj_list[1]], f_phrase[trans_adj_list[2]]], dim=1)
            instance = self.leakyrelu(torch.matmul(concat, self.W_PAA))
        elif pattern_name == 'VAA':
            concat = torch.cat([f_author[trans_adj_list[0]], f_author[trans_adj_list[1]], f_venue[trans_adj_list[2]]], dim=1)
            instance = self.leakyrelu(torch.matmul(concat, self.W_VAA))
        elif pattern_name == 'YAA':
            concat = torch.cat([f_author[trans_adj_list[0]], f_author[trans_adj_list[1]], f_year[trans_adj_list[2]]], dim=1)
            instance = self.leakyrelu(torch.matmul(concat, self.W_YAA))
        elif pattern_name == 'PPAA':
            concat = torch.cat([f_author[trans_adj_list[0]], f_author[trans_adj_list[1]], f_phrase[trans_adj_list[2]], f_phrase[trans_adj_list[3]]], dim=1)
            instance = self.leakyrelu(torch.matmul(concat, self.W_PPAA))
        else:
            concat = torch.cat([f_author[trans_adj_list[0]], f_author[trans_adj_list[1]]], dim=1)
            instance = self.leakyrelu(torch.matmul(concat, self.W_AA))
...
class InnerModel(nn.Module):
    ...
    def sample(self, adj, samples, pattern_name):
        ...
        for _ in range(self.nlayer):
            ...
            phrase_samples, venue_samples, year_samples = set(), set(), set()
            if pattern_name in self.rpt:
                for sample in sample_list[-1]:
                    ...
                    for i in samples_instance_number:
                        if pattern_name == 'PAA':
                            phrase_samples.add(adj[0][(start + i) * 2])
                            ...
                            venue_samples.add(adj[0][(start + i) * 2])
                            new_samples.add(adj[0][(start + i) * 2 + 1])
                            curr_new_adjs = np.stack(([sample], [adj[0][(start + i) * 2 + 1]], [adj[0][(start + i) * 2]]), axis=-1).tolist()
                            new_adjs.append(curr_new_adjs)
                        elif pattern_name == 'YAA':
                            year_samples.add(adj[0][(start + i) * 2])
                            new_samples.add(adj[0][(start + i) * 2 + 1])
                            curr_new_adjs = np.stack(([sample], [adj[0][(start + i) * 2 + 1]], [adj[0][(start + i) * 2]]), axis=-1).tolist()
                            new_adjs.append(curr_new_adjs)
                        elif pattern_name == 'PPAA':
                            phrase_samples.add(adj[0][(start + i) * 3])
                            phrase_samples.add(adj[0][(start + i) * 3 + 1])
                ...
                sample_list.append(np.array(list(phrase_samples)))
                sample_list.append(np.array(list(venue_samples)))
                sample_list.append(np.array(list(year_samples)))
    ...
    def transform(self, sample_list, adj_list, pattern_name):
        ...
        for i, adjs in enumerate(adj_list):
            if adjs.size != 0:
                if pattern_name in self.rpt:
                    # adjs.size != 0
                    author_index_dict, phrase_index_dict, venue_index_dict, year_index_dict = {k: v for v, k in enumerate(sample_list[1])}, {k: v for v, k in enumerate(sample_list[2])}, {k: v for v, k in enumerate(sample_list[3])}, {k: v for v, k in enumerate(sample_list[4])}
                    target_index_outs.append([base_index_dict[k] for k in adjs[0]])

                    if pattern_name == 'PAA':
                        pattern_index1, pattern_index2, pattern_index3 = [author_index_dict[k] for k in adjs[0]], [author_index_dict[k] for k in adjs[1]], [phrase_index_dict[k] for k in adjs[2]]
                        trans_adj_list.append([pattern_index1, pattern_index2, pattern_index3])
                    elif pattern_name == 'VAA':
                        pattern_index1, pattern_index2, pattern_index3 = [author_index_dict[k] for k in adjs[0]], [author_index_dict[k] for k in adjs[1]], [venue_index_dict[k] for k in adjs[2]]
                        trans_adj_list.append([pattern_index1, pattern_index2, pattern_index3])
                    elif pattern_name == 'YAA':
                        pattern_index1, pattern_index2, pattern_index3 = [author_index_dict[k] for k in adjs[0]], [author_index_dict[k] for k in adjs[1]], [year_index_dict[k] for k in adjs[2]]
                        trans_adj_list.append([pattern_index1, pattern_index2, pattern_index3])
                    elif pattern_name == 'PPAA':
                        pattern_index1, pattern_index2, pattern_index3, pattern_index4 = [author_index_dict[k] for k in adjs[0]], [author_index_dict[k] for k in adjs[1]], [phrase_index_dict[k] for k in adjs[2]], [phrase_index_dict[k] for k in adjs[3]]
    ...
    def forward(self, feats, adj, samples, embeddings_phrase, embeddings_venue, embeddings_year, pattern_name):
        ...
        author_feats = feats[sample_list[1]]

        if len(sample_list) != 2:
            phrase_feats = embeddings_phrase[sample_list[2]]
            venue_feats = embeddings_venue[sample_list[3]]
            year_feats = embeddings_year[sample_list[4]]

        for i, layer_ins_aggs in enumerate(self.ins_aggs):
            author_feats = F.dropout(author_feats, self.dropout, training=self.training)
            if pattern_name in self.rpt:
                phrase_feats = F.dropout(phrase_feats, self.dropout, training=self.training)
                venue_feats = F.dropout(venue_feats, self.dropout, training=self.training)
                year_feats = F.dropout(year_feats, self.dropout, training=self.training)
                if phrase_feats.shape[0] == 0 and venue_feats.shape[0] == 0 and year_feats.shape[0] == 0:
                    return torch.full((len(samples), self.out_dim * self.nheads[0]), 0).float().to(self.device)
                instance = torch.cat([agg(author_feats, phrase_feats, venue_feats, year_feats, trans_adj_list[-i - 1], pattern_name) for agg in layer_ins_aggs], dim=1)
            else:
                instance = torch.cat([agg(author_feats, None, None, None, trans_adj_list[-i - 1], pattern_name) for agg in layer_ins_aggs], dim=1)
...
class TEDModel(nn.Module):
    ...
    def forward(self, x, adjs, samples, embeddings_phrase, embeddings_venue, embeddings_year):
        inner_out = []
        for i, patt_agg in enumerate(self.InnerModels):
            inner_out.append(patt_agg(x, adjs[i], samples, embeddings_phrase, embeddings_venue, embeddings_year, self.pattern[i]))
