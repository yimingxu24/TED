...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='TED')
    
    parser.add_argument('--node', type=str, default='../data/DBLP/node.dat')
    parser.add_argument('--link', type=str, default='../data/DBLP/link.dat')
    parser.add_argument('--config', type=str, default='../data/DBLP/config.dat')
    parser.add_argument('--label', type=str, default='../data/DBLP/label.dat')
    parser.add_argument('--output', type=str, default='../data/DBLP/Att_Sup.emb.dat')
    parser.add_argument('--rpt', type=str, default="PAA,VAA,YAA,PPAA")
    parser.add_argument('--meta', type=str, default="1,2,3,4,5")
...
def main():
    ...
    if args.supervised=='True':
        adjs, id_name, features, phrase_attr, venue_attr, year_attr = load_data_semisupervised(args, args.node, args.link, args.config, args.rpt.split(','), list(map(lambda x: int(x), args.meta.split(','))))
    ...
    if args.attributed=='True':
        embeddings, embeddings_phrase, embeddings_venue, embeddings_year = torch.from_numpy(features).to(args.device), torch.from_numpy(phrase_attr).to(args.device), torch.from_numpy(venue_attr).to(args.device), torch.from_numpy(year_attr).to(args.device)
    elif args.supervised=='False':
        return 0
    ...
    for epoch in range(args.epochs):
        if args.supervised=='True':
            ...
            updates, pred = model(embeddings, adjs, curr_batch, embeddings_phrase, embeddings_venue, embeddings_year)
            if multi:
    ...
    with torch.no_grad():
        for index, i in enumerate(range(rounds)):
            ...
            embs, _ = model(embeddings, adjs, seed_nodes, embeddings_phrase, embeddings_venue, embeddings_year)
