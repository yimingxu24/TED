...
def parse_args():
    ...
    parser.add_argument('--node', type=str, default='../data/PubMed/node.dat')
    parser.add_argument('--link', type=str, default='../data/PubMed/link.dat')
    parser.add_argument('--config', type=str, default='../data/PubMed/config.dat')
    parser.add_argument('--label', type=str, default='../data/PubMed/label.dat')
    parser.add_argument('--output', type=str, default='../data/PubMed/Att_Sup.emb.dat')
    parser.add_argument('--rpt', type=str, default="CDD,GDD,SDD,GDCD,SDCD,SDGD")
    parser.add_argument('--meta', type=str, default="1,2,4,8")
...
def main():
    ...
    if args.supervised=='True':
        adjs, id_name, features, gene_attr, chemical_attr, species_attr = load_data_semisupervised(args, args.node, args.link, args.config, args.rpt.split(','), list(map(lambda x: int(x), args.meta.split(','))))
    ...
    if args.attributed=='True':
        embeddings, embeddings_gene, embeddings_chemical, embeddings_species = torch.from_numpy(features).to(args.device), torch.from_numpy(gene_attr).to(args.device), torch.from_numpy(chemical_attr).to(args.device), torch.from_numpy(species_attr).to(args.device)
    elif args.supervised=='False':
        return 0
    ...
    for epoch in range(args.epochs):
        if args.supervised=='True':
            ...
            updates, pred = model(embeddings, adjs, curr_batch, embeddings_gene, embeddings_chemical, embeddings_species)
            if multi:
    ...
    with torch.no_grad():
        for index, i in enumerate(range(rounds)):
            ...
            embs, _ = model(embeddings, adjs, seed_nodes, embeddings_gene, embeddings_chemical, embeddings_species)
